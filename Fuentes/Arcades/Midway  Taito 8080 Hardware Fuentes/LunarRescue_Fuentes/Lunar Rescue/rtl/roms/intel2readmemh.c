...
unsigned int address, ndata_bytes, ndata_words;
unsigned int  data;
...
int main (int argc, char *argv[])
{
   ...
   while (!feof(fpi)) {
      ...
      if (strlen(szLine) >= 10) {
         ...
         if (1)//start_address >= 0 && start_address <= 65535 && ndata_bytes > 0) 
		 {
            // Suck up data bytes starting at 9th byte.
            i = 9;

            // Words.. not bytes..
            ndata_words   = ndata_bytes;
            start_address = start_address;

			//printf ("\n%04X", start_address);
			//printf ("  %04X", ndata_words);			
			//printf ("  %04X", start_address + ndata_words);
