...
{
	{MENU_ENTRY_CALLBACK,"Reset Sinclair QL",MENU_ACTION(&Reset)},
//	{MENU_ENTRY_CYCLE,(char *)testpattern_labels,MENU_ACTION(4)},
//	{MENU_ENTRY_SUBMENU,"RGB Scaling \x10",MENU_ACTION(rgbmenu)},

	{MENU_ENTRY_CYCLE,(char *)mdv_labels,2},
	{MENU_ENTRY_CYCLE,(char *)ram_labels,2},
	{MENU_ENTRY_CYCLE,(char *)video_labels,2},
	
	{MENU_ENTRY_TOGGLE,"Scanlines",MENU_ACTION(5)},


	{MENU_ENTRY_CALLBACK,"Mount MDV \x10",MENU_ACTION(&FileSelector_Show)},
...
//	MENU_SLIDER_VALUE(&rgbmenu[2])=8;
	...
	while( 1 )
	{
		...
		dipsw = 0; //MENU_CYCLE_VALUE ( &topmenu[ 1 ] );	// Take the value of the TestPattern cycle menu entry.
...
//			dipsw |= 2;	// Add in the HQ2X bit
		...
		if(MENU_CYCLE_VALUE ( &topmenu[ 2 ] ))
			dipsw|=8;	// Add in the Border bit

		if(MENU_CYCLE_VALUE ( &topmenu[ 3 ] ))
			dipsw|=16;	// Add in the Border bit

		if( MENU_TOGGLE_VALUES & 32 )
			dipsw |=32 ;	// Add in the scanlines bit.
