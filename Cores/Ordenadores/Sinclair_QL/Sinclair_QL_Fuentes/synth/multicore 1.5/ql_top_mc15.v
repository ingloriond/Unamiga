...
begin
	...
	sram2_addr_o  <= sdram_addr[18:0]; //sensivel
	sram3_addr_o  <= sdram_addr[18:0]; //sensivel
	
	sram2_we_n_o <= 1'b1;//sensivel
	sram3_we_n_o <= 1'b1;//sensivel		
	ram_dout <= 16'hzzzz;//sensivel
			
	if (sdram_wr && sdram_ds[1])
		begin
			ram_dout[15:8] <= sdram_din[15:8]; //sensivel
			sram2_we_n_o <= 1'b0; //sensivel
		end


	if (sdram_wr && sdram_ds[0])
		begin
			ram_dout[7:0] <= sdram_din[7:0];//sensivel
			sram3_we_n_o <= 1'b0; //sensivel
		end


end

assign sdram_dout = {sram2_data_io, sram3_data_io}; //ok

assign sram2_oe_n_o	= ~(sdram_oe && sdram_ds[1]); //ok
assign sram3_oe_n_o	= ~(sdram_oe && sdram_ds[0]);//ok

assign sram2_data_io = (sram2_we_n_o) ? 8'hZZ : ram_dout[15:8];
assign sram3_data_io = (sram3_we_n_o) ? 8'hZZ : ram_dout[7:0];
...
assign vga_vsync_n_o = tv15khz?1'b1:video_vs;
assign vga_hsync_n_o = tv15khz?vga_csync:video_hs;
...
wire cpu_as;
wire cpu_rd = (cpu_as == 0 && cpu_rw == 1);
wire cpu_wr = (cpu_as == 0 && cpu_rw == 0);
wire cpu_idle = cpu_as;
...
fx68k fx68k
 (
	...
	.eRWn      ( cpu_rw ),
	.ASn       ( cpu_as ), // indicates that there is a valid address on the address bus.
	.LDSn      ( cpu_ds[0] ),
	.UDSn      ( cpu_ds[1] ),
	.E         ( ),
	.VMAn      ( ),
...
wire pll_locked, clk84, clk11, clk40;
...
//-- clock enable generation: 40/5 = 8 MHz 
reg [4:0] cpu_ce_count;
reg cpu_ce1, cpu_ce2;
...
begin
	...
	begin
		...
		if (cpu_ce_count == 5'd2)
			cpu_ce1 <= 1;
		
		if (cpu_ce_count == 5'd4)
		begin
			cpu_ce2 <= 1;
			cpu_ce_count <= 5'd0;
		end
...
assign stm_rst_o = 1'b0;//stm_rst_s;	
	...
	begin
		...
		begin
			...
			case (boot_state)
				...
				4'b0101: //clear the write signal
					begin
						dio_write <= 1'b0;
				...
				4'b1001: //clear the write signal and loop
					begin
						dio_write <= 1'b0;
						dio_addr <= dio_addr + 25'd1;
						boot_state <= 4'b0000;
...
debug_info #(100,100) debug_info (
        ...
        .b_o      ( vga_b_o   ), 
		  ...
		  .dbg1_s   ( bootdata[31:24] ),
        .dbg2_s   ( bootdata[23:16] ),
        .dbg3_s   ( bootdata[15:8]  ),
        .dbg4_s   ( bootdata[7:0]   ),
